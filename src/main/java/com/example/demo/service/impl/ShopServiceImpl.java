...
public class ShopServiceImpl implements ShopService {
	...
	public List<ShopDTO> getAllShops() {
		return shopRepository.findAll().stream().map(shop -> modelMapper.map(shop, ShopDTO.class))
	...
	public ShopDTO createShop(ShopDTO shopDTO, Integer vendorId) {
		...
		shop.setVendor(
				userRepository.findById(vendorId).orElseThrow(() -> new ResourceNotFoundException("User not found")));
	...
	public ShopDTO updateShopCommission(Integer shopId, java.math.BigDecimal commissionPercentage) {
		...
		try {
			...
			if (commissionPercentage.compareTo(java.math.BigDecimal.ZERO) < 0
				...
				logger.error("❌ [ShopServiceImpl] Invalid commission percentage: {}. Must be 0-100",
