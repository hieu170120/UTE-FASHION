...
public class ChatServiceImpl implements ChatService {
	...
	public List<MessageDTO> getMessages(int conversationId) {
		return messageRepository.findByConversationId(conversationId).stream().map(this::convertToDTO)
	...
	public Conversation findOrCreateConversation(int shopId) {
		...
		return conversationRepository.findByUserIdAndShopId(user.getUserId(), shopId).orElseGet(() -> {
	...
	private MessageDTO convertToDTO(Message message) {
		return MessageDTO.builder().id(message.getId()).conversationId(message.getConversation().getId())
				.senderId(message.getSender().getUserId()).senderType(message.getSenderType())
				.messageContent(message.getMessageContent()).isRead(message.isRead()).createdAt(message.getCreatedAt())
				.build();
	}
}
