...
public class VendorServiceImpl implements VendorService {
	...
	public Shop registerNewShop(ShopRegistrationDTO registrationDTO) {
		...
		if (shopRepository.findByVendorUserId(currentUser.getUserId()).isPresent()) {
	...
	public Optional<Shop> getCurrentVendorShop() {
		...
		return shopRepository.findByVendorUserId(currentUser.getUserId());
	}

	@Override
	@Transactional
	public Shop updateCurrentVendorShop(ShopRegistrationDTO shopDTO) {
		Shop existingShop = getCurrentVendorShop().orElseThrow(
				() -> new jakarta.persistence.EntityNotFoundException("Bạn chưa có cửa hàng để cập nhật."));
		...
		if (!existingShop.getShopName().equals(shopDTO.getShopName())
				&& shopRepository.existsByShopName(shopDTO.getShopName())) {
			throw new IllegalStateException("Tên cửa hàng đã tồn tại.");
		}
